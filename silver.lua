...
local BabyUIFolder = "BabyUI"
local ConfigurationFolder = BabyUIFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'BabyUI Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(BabyUIFolder) then
				if isfile and isfile(BabyUIFolder..'/settings'..ConfigurationExtension) then
					file = readfile(BabyUIFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"BabyUI Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"BabyUI Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('BabyUI had an issue accessing configuration saving capability.')
		end
	end
end
...
local BabyUI = useStudio and script.Parent:FindFirstChild('BabyUI') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local rayfieldDestroyed = false -- True when BabyUILibrary:Destroy() is called

repeat
	if BabyUI:FindFirstChild('Build') and BabyUI.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('BabyUI | Build Mismatch')
		print('BabyUI may encounter issues as you are running an incompatible interface version ('.. ((BabyUI:FindFirstChild('Build') and BabyUI.Build.Value) or 'No Build') ..').\n\nThis version of BabyUI is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, BabyUI = BabyUI, useStudio and script.Parent:FindFirstChild('BabyUI') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

BabyUI.Enabled = false

if gethui then
	BabyUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(BabyUI)
	BabyUI.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	BabyUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == BabyUI.Name and Interface ~= BabyUI then
			Interface.Enabled = false
			Interface.Name = "BabyUI-Old"
		end
	end
elseif not useStudio then
...
if BabyUI.AbsoluteSize.X < minSize.X and BabyUI.AbsoluteSize.Y < minSize.Y then
...
local Main = BabyUI.Main
local MPrompt = BabyUI:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = BabyUI:FindFirstChild('Drag')
...
BabyUI.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/BabyUI/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = BabyUI.Notifications

local SelectedTheme = BabyUILibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = BabyUILibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	BabyUI.Main.BackgroundColor3 = SelectedTheme.Background
	BabyUI.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	BabyUI.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	BabyUI.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	BabyUI.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	BabyUI.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	BabyUI.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		BabyUI.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		BabyUI.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(BabyUI:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("BabyUI | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("BabyUI | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('BabyUI had an issue decoding the configuration file, please try delete the file and reopen BabyUI.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(BabyUILibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("BabyUI | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--BabyUILibrary:Notify({Title = "BabyUI Flags", Content = "BabyUI was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(BabyUILibrary.Flags) do
...
function BabyUILibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('BabyUI | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			BabyUILibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			BabyUILibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "rayfieldOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(BabyUIFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('BabyUI Settings', 0, true)

	if TabList['BabyUI Settings'] then
		TabList['BabyUI Settings'].LayoutOrder = 1000
	end

	if Elements['BabyUI Settings'] then
		Elements['BabyUI Settings'].LayoutOrder = 1000
	end
...
function BabyUILibrary:CreateWindow(Settings)
	print('creating window')
	if BabyUI:FindFirstChild('Loading') then
		if getgenv and not getgenv().rayfieldCached then
			BabyUI.Enabled = true
			BabyUI.Loading.Visible = true

			task.wait(1.4)
			BabyUI.Loading.Visible = false
		end
	end
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				BabyUILibrary:Notify({Title = 'Build Mismatch', Content = 'BabyUI may encounter issues as you are running an incompatible interface version ('.. ((BabyUI:FindFirstChild('Build') and BabyUI.Build.Value) or 'No Build') ..').\n\nThis version of BabyUI is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if isfolder and not isfolder(BabyUIFolder) then
		makefolder(BabyUIFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "BabyUI"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "BabyUI Interface Suite" then
		LoadingFrame.Version.Text = "BabyUI UI"
	end
	...
	if not Settings.DisableBabyUIPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				BabyUILibrary:Notify({
					Title = "BabyUI Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(BabyUIFolder.."/Discord Invites") then
			makefolder(BabyUIFolder.."/Discord Invites")
		end

		if isfile and not isfile(BabyUIFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(BabyUIFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"BabyUI RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(BabyUIFolder.."/Key System") then
			makefolder(BabyUIFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("BabyUI | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BabyUI specific development.')
				end
			end
		end
		...
		if isfile and isfile(BabyUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(BabyUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(BabyUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						BabyUILibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called BabyUILibrary:Destroy()
				...
				if not Success then
					...
					print("BabyUI | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BabyUI specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= BabyUILibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					BabyUILibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			BabyUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("BabyUI | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BabyUI specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					BabyUILibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("BabyUI | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with BabyUI specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					BabyUILibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("BabyUI | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with BabyUI specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					BabyUILibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= BabyUILibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("BabyUI | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BabyUI specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						BabyUILibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("BabyUI | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with BabyUI specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					BabyUILibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			BabyUILibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			BabyUILibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('BabyUI had an issue creating settings.') end
...
function BabyUILibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function BabyUILibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function BabyUILibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['BabyUI Settings'])
		end)
	end)
...
function BabyUILibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				BabyUILibrary:Notify({Title = "BabyUI Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			BabyUILibrary:Notify({Title = "BabyUI Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('BabyUI Configurations Error | '..tostring(result))
			BabyUILibrary:Notify({Title = "BabyUI Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = BabyUILibrary:CreateWindow({
	--	Name = "BabyUI Example Window",
	--	LoadingTitle = "BabyUI Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using BabyUI may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like BabyUI to get the key from
	...
	----BabyUILibrary:Notify({Title = "BabyUI Interface", Content = "Welcome to BabyUI. These - are the brand new notification design for BabyUI, with custom sizing and BabyUI calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(BabyUILibrary.Theme) do
...
task.delay(4, function()
	BabyUILibrary.LoadConfiguration()
